	...
	function dt_sc_iconbox($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-icon-box {$type} {$class}'{$style}>";
			...
			if( !empty($iconurl) && ( $icon_type == 'image' ) ):
				...
				if( $type == 'type2' ) {					
					if( !empty($hover_iconurl) && ( $icon_type == 'image' ) ) {						
						$image = wpb_getImageBySize( array( 'attach_id' => $hover_iconurl, 'thumb_size' => 'full' ));
						$out .= '<span class="hover-image">'.$image['thumbnail'].'</span>';
					}
				}
	...
	function dt_sc_image( $attrs, $content = null ) {
		...
		if( !empty( $url ) ):
			...
			if( strlen( $link['url'] ) > 0 ) {
				return '<div class="dt-sc-any-image-wrapper"> <a href="'.$a_href.'" title="'.$a_title.'" target="'.$a_target.'">'.$url.'</a> </div>';
			} else {
				return '<div class="dt-sc-any-image-wrapper">'.$url.'</div>';				
			}
		endif;
	}
...
function dt_sc_tm_carousel_item($attrs, $content = null) {
			...
			$out .= "<div class='dt-sc-testimonial {$type} {$class}'>";
				...
				$img = '<div class="dt-sc-testimonial-author">';
					...
					if( $i_type == 'type1' ) {
						...
						$img .= !empty( $name ) ? '<cite>'.$name.'</cite>' : '';
						$img .= !empty( $location ) ? '<span class="location">'.$location.'</span>' : '';
					}
				...
				$con = '<div class="dt-sc-testimonial-quote">';
					...
					if( $i_type == 'type2' ) {
						...
						$rating = isset( $rating )? $rating : 'star-3';
						$con .= '<span class="'.$rating.'"></span>';
						$con .=  !empty( $title ) ? '<h2>'.$title.'</h2>' : '';
					}
	...
	function dt_sc_post( $attrs, $content = null ){
		...
		if( $the_query->have_posts() ):
			while( $the_query->have_posts() ):
				...
				$post_classes = get_post_class( array('blog-entry','format-'.$format, $style) , $id );
						...
						if( !empty( $post_meta['post-gallery-items'] ) ) :
							...
							$out .= '	<ul class="entry-gallery-post-slider">';
											foreach ($ids as $id ) {
												...
												$out .= "<li><img src='". esc_url($attachment[0])."' alt=''/></li>";
											}
							$out .= '	</ul>';
							$out .= '	<div class="entry-format '.esc_attr($hide_post_format).'">';
							$out .= '		<a class="ico-format" href="'.esc_url(get_post_format_link( $format )).'"></a>';
					...
					elseif( $format === "video" ) :
						...
						if( !empty( $media ) ) :
							...
							$out .= '	<div class="dt-video-wrap">';
											if( $type == 'oembed' ):
												$out .= wp_oembed_get($media);
											elseif( $type == 'self' ) :
												$out .= wp_video_shortcode( array('src' => $media ) );
											endif;
							$out .='	</div>';
							$out .= '	<div class="entry-format '.esc_attr($hide_post_format).'">';
							$out .= '		<a class="ico-format" href="'.esc_url(get_post_format_link( $format )).'"></a>';
						...
						if( !empty( $media ) ) :
							$out .= '<div class="entry-thumb">';
								if( $type == 'oembed' ):
									$out .= wp_oembed_get( $media );
								elseif( $type == 'self' ) :
									$out .= wp_audio_shortcode( array('src' => $media ) );
								endif;
							$out .= '	<div class="entry-format '.esc_attr($hide_post_format).'">';
							$out .= '		<a class="ico-format" href="'.esc_url( $format_link ).'"></a>';
							$out .= '	</div>';
							$out .= '</div>';						
						elseif( has_post_thumbnail() ):	
							$out .= '<div class="entry-thumb">';
					...
					if( $style == "entry-date-left") :
						...
						$out .= '	</div>';
									...
									if( strtolower($allow_excerpt) == 'yes' ):
										$out .= '<div class="entry-body">'.mount_resort_excerpt($excerpt_length).'</div>';
									endif;
		...
		if( !empty( $id ) ){
			...
			if( $p->post_type === "dt_portfolios" ) {
				...
				if( !empty($portfolio_item_meta['portfolio-gallery']) ) {
					...
					$items = true;
					$galleries = explode(',', $portfolio_item_meta["portfolio-gallery"]);

					$popup = wp_get_attachment_image_src($galleries[0], 'full', false);
					$popup = $popup[0];

					if( sizeof($galleries) > 1 ){
						$popup = wp_get_attachment_image_src($galleries[1], 'full', false);
						$popup = $popup[0];
					}

					$image = wp_get_attachment_image_src($galleries[0], 'full', false);
					$image = $image[0];					
				}
	...
	function dt_sc_down_count($attrs, $content = null) {
		...
		$out .= '<div class="downcount '.$class.'" data-date="'.mount_resort_cs_get_option('comingsoon-launchdate', date('m/d/Y')).'" data-offset="'.mount_resort_cs_get_option('comingsoon-timezone', '0').'">';
