...
if(!empty($wtstyle)):
	...
	if( is_page() ):
		...
		mount_resort_show_sidebar('page',$page_id, 'right');
	elseif( is_single() ):
	
		if( is_singular('post') ) {
	
			$id = ($post->ID == 0) ? get_queried_object_id() : $post->ID;
			mount_resort_show_sidebar('post',$id, 'right');
		} elseif( is_singular('dt_portfolios') ) {
	
			$id = ($post->ID == 0) ? get_queried_object_id() : $post->ID;
			mount_resort_show_sidebar('dt_portfolios',$id, 'right');
			...
			mount_resort_show_sidebar('',$id, 'right');
		}	
	elseif( class_exists('woocommerce') && is_post_type_archive('product') ):
		mount_resort_show_sidebar('page',get_option('woocommerce_shop_page_id'), 'right');
