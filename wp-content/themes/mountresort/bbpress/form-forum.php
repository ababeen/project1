			...
			<fieldset class="bbp-form">
				<legend>
					...
					<?php
						if ( bbp_is_forum_edit() )
							printf( __( 'Now Editing &ldquo;%s&rdquo;', 'mountresort' ), bbp_get_forum_title() );
						else
							bbp_is_single_forum() ? printf( __( 'Create New Forum in &ldquo;%s&rdquo;', 'mountresort' ), bbp_get_forum_title() ) : esc_html_e('Create New Forum', 'mountresort' );
					?>
				...
				<?php if ( !bbp_is_forum_edit() && bbp_is_forum_closed() ) : ?>
					...
					<div class="bbp-template-notice">
						<p><?php esc_html_e('This forum is closed to new content, however your account still allows you to do so.', 'mountresort' ); ?></p>
					</div>
				...
				<div>
					...
					<p>
						<label for="bbp_forum_title"><?php printf( __( 'Forum Name (Maximum Length: %d):', 'mountresort' ), bbp_get_title_max_length() ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_type"><?php esc_html_e('Forum Type:', 'mountresort' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_status"><?php esc_html_e('Status:', 'mountresort' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_visibility"><?php esc_html_e('Visibility:', 'mountresort' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_parent_id"><?php esc_html_e('Parent Forum:', 'mountresort' ); ?></label><br />

						<?php
							bbp_dropdown( array(
								'select_id' => 'bbp_forum_parent_id',
								'show_none' => esc_html__( '(No Parent)', 'mountresort' ),
					...
					<div class="bbp-submit-wrapper">
						...
						<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_forum_submit" name="bbp_forum_submit" class="button submit"><?php esc_html_e('Submit', 'mountresort' ); ?></button>
...
<?php elseif ( bbp_is_forum_closed() ) : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php printf( __( 'The forum &#8216;%s&#8217; is closed to new content.', 'mountresort' ), bbp_get_forum_title() ); ?></p>
		</div>
	</div>
...
<?php else : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php is_user_logged_in() ? esc_html_e('You cannot create new forums.', 'mountresort' ) : esc_html_e('You must be logged in to create new forums.', 'mountresort' ); ?></p>
		</div>
	</div>
