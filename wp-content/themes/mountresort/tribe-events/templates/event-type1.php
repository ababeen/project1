    ...
    <p class="nav-top-links">
        <a href="<?php echo esc_url( tribe_get_events_link() ); ?>" class="back-nav"><?php esc_html_e('All Events', 'mountresort'); ?></a>
        <span class="sep">/</span>
        <a href="<?php echo esc_url( tribe_get_gcal_link($event_id) ); ?>"><?php esc_html_e('+ Google Calendar', 'mountresort'); ?></a>
        <span class="sep">/</span>
        <a href="<?php echo esc_url( tribe_get_ical_link() ); ?>"><?php esc_html_e('+ iCal Export', 'mountresort'); ?></a>
    </p>
	<h2><?php the_title(); ?></h2>
    ...
    <p class="event-schedule"><?php
      ...
      if(class_exists( 'Tribe__Events__Pro__Main' )): ?>
          | <a href="#" class="dt-sc-tooltip-top" title="<?php echo tribe_get_recurrence_text ( $event_id ); ?>"><?php esc_html_e('Recurring Event (See all)', 'mountresort'); ?></a><?php
      endif; ?>
    </p>
    ...
    <div class="dt-sc-two-third column first">
      <div class="event-image-wrapper">
          ...
          <?php echo tribe_event_featured_image( $event_id, 'mount-resort-event-single-type1', false ); ?>
      </div>    
    ...
    <div class="dt-sc-one-third column">
        <h3><?php esc_html_e('Details', 'mountresort'); ?></h3>
        <ul class="event-details">
            <li><dt><?php esc_html_e('Start:', 'mountresort'); ?></dt><?php echo tribe_get_start_date ( $event_id, true, 'M d' ).' @ '.tribe_get_start_time ( $event_id, 'h:i a' ); ?></li>
            <li><dt><?php esc_html_e('End:', 'mountresort'); ?></dt><?php echo tribe_get_end_date ( $event_id, true, 'M d' ).' @ '.tribe_get_end_time ( $event_id, 'h:i a' ); ?></li>
            <?php if ( tribe_get_cost() ) : ?>
                <li><dt><?php esc_html_e('Cost:', 'mountresort'); ?></dt><?php echo tribe_get_cost( $event_id, true ); ?></li>
            <?php endif; ?>
            ...
            if(!empty($website)): ?>
                <li><dt><?php esc_html_e('Website:', 'mountresort'); ?></dt><?php echo ($website); ?></li><?php
            endif; ?>
        </ul>
        <div class="dt-sc-hr-invisible-xsmall"></div>
        <h3><?php esc_html_e('Organizer', 'mountresort'); ?></h3>
        <h4><?php
        if(class_exists( 'Tribe__Events__Pro__Main' ))
        ...
        <ul class="event-organize"><?php
            ...
            if(!empty($phone)): ?>
                <li><dt><?php esc_html_e('Phone:', 'mountresort'); ?></dt><?php echo ($phone); ?></li><?php
            endif;
            $email = tribe_get_organizer_email();
            if(!empty($email)): ?>
                <li><dt><?php esc_html_e('Email:', 'mountresort'); ?></dt><a href="mailto:<?php echo esc_attr($email); ?>"><?php echo ($email); ?></a></li><?php
            endif;
        ...
        <div class="dt-sc-hr-invisible-xsmall"></div>
        <h3><?php esc_html_e('Venue', 'mountresort'); ?></h3>
        <h4><?php
        if(class_exists( 'Tribe__Events__Pro__Main' ))
