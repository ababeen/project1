...
get_header();
    ...
    <section id="primary" class="<?php echo esc_attr( $page_layout );?>"><?php
		...
		$show_date_meta = $tpl_default_settings['show-date-info'] ? "" : "hidden";
		$show_comment_meta = $tpl_default_settings['show-comment-info'] ? "comments" : "hidden";
		$show_author_meta = $tpl_default_settings['show-author-info'] ? "" : "hidden";
		$show_category_meta = $tpl_default_settings['show-category-info'] ? "" : "hidden";
		$show_tag_meta = $tpl_default_settings['show-tag-info']  ? "" : "hidden";
		$show_post_format = $tpl_default_settings['show-postformat-info'] ? "" : "hidden";
		$container_class = "";
		...
		if( $the_query->have_posts() ):
			...
			while( $the_query->have_posts() ):
                ...
                $format = $post_meta['post-format-type'];
                $format_link =  get_post_format_link( $format );?>
                <div class="<?php echo esc_attr($temp_class);?>">
                    <article id="post-<?php the_ID();?>" <?php post_class(array( 'blog-entry', 'format-'.$format, $post_style));?>>
                        <!-- Featured Image -->
                        <?php if( $format == "image" || empty($format) ) :
                                if( has_post_thumbnail() ) :?>
                                    <div class="entry-thumb">
                                        <a href="<?php the_permalink();?>" title="<?php printf(esc_attr__('Permalink to %s','mountresort'),the_title_attribute('echo=0'));?>"><?php the_post_thumbnail("full");?></a>
                            ...
                            elseif( $format === "gallery" ) :
                                if( !empty( $post_meta['post-gallery-items'] ) ):
                                    $ids = explode( ',', $post_meta['post-gallery-items'] );
                            ...
                            elseif( $format === "video" ) :
                                $type = $post_meta['audio-or-video-post'];
                                $media = $post_meta['media-url'];
                                ...
                                if( !empty( $media ) ) :
                                    ...
                                    echo'   <div class="dt-video-wrap">';
                                                ...
                                                elseif( $type == 'self' ):
                                                    echo wp_video_shortcode( array( 'src' => $media ) );
                                                endif;
                                    echo '  </div>';
                                    echo '  <div class="entry-format '.esc_attr($show_post_format).'">';
                                    echo '      <a class="ico-format" href="'.esc_url(get_post_format_link( $format )).'"></a>';
                                ...
                                if( !empty( $media ) ) :
                                    echo '<div class="entry-thumb">';
                                            if( $type == 'oembed' ):
                                                echo wp_oembed_get( $media );
                                            elseif( $type == 'self' ):
                                                echo wp_audio_shortcode( array( 'src' => $media ) );
                                            endif;                    
                                    echo '  <div class="entry-format '.esc_attr($show_post_format).'">';
                                    echo '      <a class="ico-format" href="'.esc_url(get_post_format_link( $format )).'"></a>';
                                    echo '  </div>';
                        ...
                        <?php if( $post_style == "entry-date-left"): ?>
                                ...
                                <div class="entry-details">
                                    ...
                                    <div class="entry-title">
                                        <h4><a href="<?php the_permalink();?>" title="<?php printf(esc_attr__('Permalink to %s','mountresort'), the_title_attribute('echo=0'));?>"><?php the_title(); ?></a></h4>
                                    </div>
                                    
                                    <?php if( $tpl_default_settings['blog-post-excerpt'] && array_key_exists('blog-post-excerpt-length',$tpl_default_settings) ):?>
                                            <div class="entry-body"><?php echo mount_resort_excerpt($tpl_default_settings['blog-post-excerpt-length']);?></div>
                                   <?php endif;?>
                                   ...
                                   <div class="entry-meta-data <?php echo esc_attr($tclass);?>">
                                        ...
                                        <p class="author <?php echo esc_attr( $show_author_meta );?>">
                                            ...
                                            <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                                                title="<?php esc_attr_e('View all posts by ', 'mountresort'); echo get_the_author();?>"><?php echo get_the_author();?></a>
                                        </p>
                        ...
                        <?php elseif( $post_style == "entry-date-author-left"):
                                ...
                                <div class="entry-date-author <?php echo esc_attr($tclass);?>">
                                    ...
                                    <div class="entry-author <?php echo esc_attr( $show_author_meta );?>">
                                        ...
                                        <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                                            title="<?php esc_attr_e('View all posts by ', 'mountresort'); echo get_the_author();?>"><span><?php echo get_the_author();?></span></a>
                                    </div>
                        ...
                        <?php else: # Default Post Style ?>
                                <div class="entry-details">
                                    ...
                                    <div class="entry-meta <?php echo esc_attr($tclass);?>">
                                        ...
                                        <div class="date <?php echo esc_attr($show_date_meta);?>"><?php esc_html_e('Posted on','mountresort'); echo get_the_date(' d M Y');?></div>
        ...
        <!-- **Pagination** -->
        <div class="pagination blog-pagination"><?php echo mount_resort_pagination($the_query); ?></div><!-- **Pagination** -->
        <!-- Blog Template Ends -->
