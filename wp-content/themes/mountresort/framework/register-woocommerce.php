...
// Product has Gallery -------------------------------------------------------
if(!is_admin())
	add_filter( 'post_class', 'mount_resort_product_has_gallery' );

function mount_resort_product_has_gallery( $classes ) {
	global $product;
...
// Change Image Sizes --------------------------------------------------------
$pagenow = mount_resort_global_variables('pagenow');
if ( is_admin() && isset( $_GET['activated'] ) && $pagenow == 'themes.php' ) add_action( 'init', 'mount_resort_woo_image_dimensions', 1 );
function mount_resort_woo_image_dimensions() {
...
add_filter( 'loop_shop_columns', 'mount_resort_woo_loop_columns' );
if (!function_exists('mount_resort_woo_loop_columns')) {
	function mount_resort_woo_loop_columns() {
...
add_filter( 'loop_shop_per_page', 'mount_resort_woo_product_count' );
if (!function_exists('mount_resort_woo_product_count')) {
	function mount_resort_woo_product_count() {
...
// Hide page title -----------------------------------------------------------
add_action( 'woocommerce_show_page_title', 'mount_resort_woo_show_page_title', 10);
if( !function_exists('mount_resort_woo_show_page_title') ) {
	function mount_resort_woo_show_page_title() {
		return false;
	}
}

// Before main content -------------------------------------------------------
add_action( 'woocommerce_before_main_content', 'mount_resort_woo_before_main_content', 10);
if( !function_exists('mount_resort_woo_before_main_content') ) {
	function mount_resort_woo_before_main_content() {
		
		if( is_shop() ):
			// Page Settings
			$tpl_default_settings = get_post_meta( get_option('woocommerce_shop_page_id') ,'_dt_page_settings',TRUE);
...
// After main content -------------------------------------------------------
add_action( 'woocommerce_after_main_content', 'mount_resort_woo_after_main_content', 20);
if( !function_exists('mount_resort_woo_after_main_content') ) {
	function mount_resort_woo_after_main_content() {
 ...
 * --------------------------------------------------------------------------- */
add_action( 'woocommerce_before_subcategory', 'mount_resort_woo_before_subcategory', 5);
function mount_resort_woo_before_subcategory() {
...
add_action( 'woocommerce_before_subcategory_title', 'mount_resort_woocommerce_before_subcategory_title', 5);
function mount_resort_woocommerce_before_subcategory_title() {
	echo '<div class="product-thumb"><span class="image">';
}

remove_action('woocommerce_shop_loop_subcategory_title', 'woocommerce_template_loop_category_title', 10);
add_action( 'woocommerce_after_subcategory_title', 'mount_resort_woocommerce_after_subcategory_title', 10);
function mount_resort_woocommerce_after_subcategory_title( $category ) {
	echo '</span></div>';
...
add_action( 'woocommerce_after_subcategory', 'mount_resort_woo_after_subcategory', 10);
function mount_resort_woo_after_subcategory() {
	echo '</div></div>';
}
 ...
 * --------------------------------------------------------------------------- */
add_action( 'woocommerce_before_shop_loop_item', 'mount_resort_woo_shop_overview_extra_div', 5);
function mount_resort_woo_shop_overview_extra_div() {
...
add_action( 'woocommerce_before_shop_loop_item_title', 'mount_resort_woo_show_product_loop_sale_flash', 10 );
function mount_resort_woo_show_product_loop_sale_flash() {
	global $product;
	$out = "";
	if( $product->is_on_sale() and $product->is_in_stock() )
		$out = '<span class="onsale"><span>'.esc_html__('Sale','mountresort').'</span></span>';

	elseif(!$product->is_in_stock())
		$out = '<span class="out-of-stock"><span>'.esc_html__('Out of Stock','mountresort').'</span></span>';

	if( $product->is_featured())
		$out .= '<div class="featured-tag"><div><i class="fa fa-thumb-tack"></i><span>'.esc_html__('Featured','mountresort').'</span></div></div>';
...
add_action( 'woocommerce_after_shop_loop_item', 'mount_resort_woo_shop_overview_show_price', 10);
function mount_resort_woo_shop_overview_show_price() {
...
add_action( 'woocommerce_after_shop_loop_item', 'mount_resort_woo_shop_overview_extra_div_close', 10);
function mount_resort_woo_shop_overview_extra_div_close() {
...
// Pagination hook ----------------------------------------------------------
add_action( 'woocommerce_after_shop_loop', 'mount_resort_woo_after_shop_loop', 10);
function mount_resort_woo_after_shop_loop() { ?>
    <div class="pagination">
        <?php if(function_exists("mount_resort_pagination")) echo mount_resort_pagination(); ?>
    </div><?php
}
...
add_action( 'woocommerce_after_single_product_summary', 'mount_resort_woo_output_related_products', 20);
function mount_resort_woo_output_related_products() {
...
add_action( 'woocommerce_after_single_product_summary', 'mount_resort_woo_output_upsells', 21); // needs to be called after the "related product" function to inherit columns and product count
function mount_resort_woo_output_upsells() {
...
add_action('woocommerce_before_single_product_summary','mount_resort_woo_show_product_sale_flash',10);
function mount_resort_woo_show_product_sale_flash() {
	global $product;
	$out = "";
...
add_action('woocommerce_after_single_product_summary','mount_resort_woo_close_product_wrapper',10);
function mount_resort_woo_close_product_wrapper() {
	$out = '</div>';
	echo ($out);
}

add_action( 'woocommerce_product_thumbnails', 'mount_resort_woo_show_product_thumbnails', 30 );
function mount_resort_woo_show_product_thumbnails() {
	...
	elseif(!$product->is_in_stock())
		$out .= '<span class="out-of-stock">'.esc_html__('Out of Stock','mountresort').'</span>';
