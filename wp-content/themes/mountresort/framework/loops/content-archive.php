<?php
	...
	if( have_posts() ) :
		...
		while( have_posts() ):
            ...
            $format =  array_key_exists( 'post-format-type', $post_meta ) ? $post_meta['post-format-type'] : 'standard';
                        ...
                        <?php if( $post_style == "entry-date-left"): ?>
                                ...
                                <div class="entry-details">
                                   ...
                                   <?php if( !empty($allow_excerpt) && !empty($excerpt) ):?>
                                    		<div class="entry-body"><?php echo mount_resort_excerpt($excerpt);?></div>
                                   <?php endif;?>
                                   ...
                                   <?php if( !empty($allow_read_more) && !empty($read_more) ):
								   			if( shortcode_exists( 'dt_sc_button' ) && mount_resort_is_plugin_active('js_composer/js_composer.php') ):
												$read_more = stripcslashes($read_more);
												$sc = str_replace("]",' link="url:'.$link.'"]',$read_more);
												$sc = do_shortcode($sc);
												echo !empty( $sc ) ? $sc : '';
											else:
												echo '<a href="'.get_permalink().'" title="'.get_the_title().'" class="dt-sc-button small icon-right with-icon filled dt-sc-readmore-link">'.esc_html__('Read More', 'mountresort').'<span class="fa fa-long-arrow-right"> </span></a>';
											endif;
                        ...
                        <?php else: # Default Post Style ?>
                        		<div class="entry-details">
                                    ...
                                    <div class="entry-meta">
                                    	...
                                    	<div class="date <?php echo esc_attr($show_date_meta);?>"><?php esc_html_e('Posted on','mountresort'); ?>  <?php echo get_the_date ( get_option('date_format') );?></div>
        ...
        <!-- **Pagination** -->
        <div class="pagination blog-pagination"><?php echo mount_resort_pagination();?></div><!-- **Pagination** -->
        <!-- Blog Template Ends --><?php

	else:?>
    	<h2><?php esc_html_e('Nothing Found.', 'mountresort'); ?></h2>
        <p><?php esc_html_e('Apologies, but no results were found for the requested archive.', 'mountresort'); ?></p><?php
	endif;?>