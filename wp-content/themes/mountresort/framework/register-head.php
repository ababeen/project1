 ...
 * --------------------------------------------------------------------------- */
add_action('wp_enqueue_scripts', 'mount_resort_enqueue_scripts');
function mount_resort_enqueue_scripts() {
	...
	// scipts variable -----------------------------------------------------------
	$loadingbar = (int) get_theme_mod( 'use-site-loader', mount_resort_defaults('use-site-loader') );
	$loadingbar = !empty( $loadingbar ) ?  "enable" : "disable";
	
	$stickynav = (int) get_theme_mod( 'enable-sticy-nav', mount_resort_defaults('enable-sticy-nav') );
	...
	wp_enqueue_script('mount-resort-jqplugins', get_theme_file_uri('/framework/js/jquery.plugins.js'), array(), false, true);
	...
	wp_enqueue_script('mount-resort-jqcustom', get_theme_file_uri('/framework/js/custom.js'), array(), false, true);

	wp_localize_script('mount-resort-jqplugins', 'dttheme_urls', array(
		'theme_base_url' => esc_js(MOUNT_RESORT_THEME_URI),
		'framework_base_url' => esc_js(MOUNT_RESORT_THEME_URI).'/framework/',
		...
		'advOptions' => esc_html__('Show Advanced Options', 'mountresort'),
	));
	...
	if( isset($picker) ) {
		...
		wp_enqueue_script('mount-resort-jqcpanel', get_theme_file_uri('/framework/js/controlpanel.js'),array(),false,true);
	}
}
...
* --------------------------------------------------------------------------- */
function mount_resort_viewport() {
	$enable = (int) get_theme_mod( 'site-responsive', mount_resort_defaults('site-responsive') );
	if( $enable ){
...
* --------------------------------------------------------------------------- */
function mount_resort_scripts_custom() {
	
	$enable_custom_js = (int) get_theme_mod( 'enable-custom-js', mount_resort_defaults('enable-custom-js') );
	$custom_js = get_theme_mod( 'custom-js', '');
	
	if( !empty( $enable_custom_js ) && !empty( $custom_js ) ){
		wp_add_inline_script('mount-resort-jqcustom', mount_resort_wp_kses(stripslashes($custom_js)) ,'after');
	}
}
add_action('wp_enqueue_scripts', 'mount_resort_scripts_custom', 100);

/* ---------------------------------------------------------------------------
 * Loading Theme Styles
 * --------------------------------------------------------------------------- */
add_action( 'wp_enqueue_scripts', 'mount_resort_enqueue_styles', 101 );
function mount_resort_enqueue_styles() {

	// site icons ---------------------------------------------------------------
	if ( ! has_site_icon() ):
		$url = MOUNT_RESORT_THEME_URI . "/images/favicon.ico";
		echo "<link href='$url' rel='shortcut icon' type='image/x-icon' />\n";		
	endif;

	// wp_enqueue_style ---------------------------------------------------------------
	wp_enqueue_style( 'mountresort', get_stylesheet_uri(), false, MOUNT_RESORT_THEME_VERSION, 'all' );

	wp_enqueue_style( 'mount-resort-base',		  get_theme_file_uri('/css/base.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );
	wp_enqueue_style( 'mount-resort-grid', 		  get_theme_file_uri('/css/grid.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );
	wp_enqueue_style( 'mount-resort-widget', 	  get_theme_file_uri('/css/widget.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );
	wp_enqueue_style( 'mount-resort-layout', 	  get_theme_file_uri('/css/layout.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );
	wp_enqueue_style( 'mount-resort-blog',	      get_theme_file_uri('/css/blog.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );
	wp_enqueue_style( 'mount-resort-portfolio',	  get_theme_file_uri('/css/portfolio.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );
	wp_enqueue_style( 'mount-resort-contact',	  get_theme_file_uri('/css/contact.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );
	wp_enqueue_style( 'mount-resort-custom-class', get_theme_file_uri('/css/custom-class.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );

	wp_enqueue_style( 'prettyphoto',	get_theme_file_uri('/css/prettyPhoto.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );
	...
	if( cs_get_option( 'enable-comingsoon' ) )
		wp_enqueue_style("mount-resort-comingsoon",  get_theme_file_uri("/css/comingsoon.css"), false, MOUNT_RESORT_THEME_VERSION, 'all' );

	// notfound css
	if ( is_404() )
		wp_enqueue_style("mount-resort-notfound",	get_theme_file_uri("/css/notfound.css"), false, MOUNT_RESORT_THEME_VERSION, 'all' );

	// loader css
	$loadingbar = (int) get_theme_mod( 'use-site-loader', mount_resort_defaults('use-site-loader') );
	if( !empty( $loadingbar ) )
		wp_enqueue_style("mount-resort-loader", 		get_theme_file_uri("/css/loaders.css"), false, MOUNT_RESORT_THEME_VERSION, 'all' );

	// woocommerce css
	if( function_exists( 'is_woocommerce' ) )
		wp_enqueue_style( 'mount-resort-woo', 		get_theme_file_uri('/css/woocommerce.css'), 'woocommerce-general-css', MOUNT_RESORT_THEME_VERSION, 'all' );

	// skin css
	$use_predefined_skin = (int) get_theme_mod( 'use-predefined-skin', mount_resort_defaults('use-predefined-skin') );
	if( !empty( $use_predefined_skin ) ) :
			$skin = get_theme_mod( 'predefined-skin', mount_resort_defaults('predefined-skin') );
			wp_enqueue_style("skin", 	get_theme_file_uri("/css/skins/$skin/style.css"));
	endif;	

	// tribe-events -------------------------------------------------------------------
	wp_enqueue_style( 'mount-resort-customevent', 		get_theme_file_uri('/tribe-events/custom.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );

	// responsive ---------------------------------------------------------------------
	$responsive = (int) get_theme_mod( 'site-responsive', mount_resort_defaults('site-responsive') );
	if( !empty( $responsive ) )
		wp_enqueue_style( "mount-resort-responsive",  	get_theme_file_uri('/css/responsive.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );

	// custom css ---------------------------------------------------------------------
	wp_enqueue_style( 'mount-resort-custom', 			get_theme_file_uri('/css/custom.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );

	// jquery scripts --------------------------------------------
	wp_enqueue_script('modernizr-custom', 	get_theme_file_uri('/framework/js/modernizr.custom.js'), array('jquery'));

	// rtl ----------------------------------------------------------------------------
	if(is_rtl() || isset( $_REQUEST['rtl'] )) wp_enqueue_style('mount-resort-rtl', 	get_theme_file_uri('/css/rtl.css'), false, MOUNT_RESORT_THEME_VERSION, 'all' );

	# Sub Menu Wrapper Shadow
	$enable_sub_menu_shadow = (int) get_theme_mod( 'allow-sub-menu-box-shadow', mount_resort_defaults('allow-sub-menu-box-shadow') );
	if( !empty( $enable_sub_menu_shadow ) ) {

		$h_shadow = (int) get_theme_mod( 'sub-menu-box-h-shadow', mount_resort_defaults('sub-menu-box-h-shadow') );
		$h_shadow .= 'px ';

		$v_shadow = (int) get_theme_mod( 'sub-menu-box-v-shadow', mount_resort_defaults('sub-menu-box-v-shadow') );
		$v_shadow .= 'px ';

		$blur_shadow = (int) get_theme_mod( 'sub-menu-box-blur-shadow', mount_resort_defaults('sub-menu-box-blur-shadow') );
		$blur_shadow .= 'px ';

		$spread_shadow = (int) get_theme_mod( 'sub-menu-box-spread-shadow', mount_resort_defaults('sub-menu-box-spread-shadow') );
		$spread_shadow .= 'px ';

		$color_shadow = get_theme_mod( 'sub-menu-box-shadow-color', '' );

		$inset_shadow = (int) get_theme_mod( 'sub-menu-box-shadow-inset', mount_resort_defaults('sub-menu-box-shadow-inset') );
		...
		wp_add_inline_style( 'mountresort', $css );		
	}

	# Sub Menu Wrapper Gradient BG
	$enable_sub_menu_bg = (int) get_theme_mod( 'allow-sub-menu-bg-color', mount_resort_defaults('allow-sub-menu-bg-color') );
	$sub_menu_bg_type = get_theme_mod( 'sub-menu-bg-color-type', mount_resort_defaults('sub-menu-bg-color-type') );
	...
	$use_predefined_skin = (int) get_theme_mod( 'use-predefined-skin', mount_resort_defaults('use-predefined-skin') );
	$primary_color = get_theme_mod('primary-color',mount_resort_defaults('primary-color'));
	$secondary_color = get_theme_mod('secondary-color',mount_resort_defaults('secondary-color'));
	$tertiary_color = get_theme_mod('tertiary-color',mount_resort_defaults('tertiary-color'));
	...
	if( empty( $use_predefined_skin ) ) {
		...
		if( !empty( $primary_color ) ) {
			...
			$rgba = mount_resort_hex2rgb( $primary_color );
			$rgba = implode(',', $rgba);

			# Header
			$header_type = get_theme_mod('header-type',mount_resort_defaults('header-type'));
			...
			$css .= '.portfolio .image-overlay, .recent-portfolio-widget ul li a:before, .dt-sc-image-caption.type2:hover .dt-sc-image-content, .dt-sc-fitness-program-short-details-wrapper .dt-sc-fitness-program-short-details, /* New */ .dt-sc-image-caption.type5 h3, .dt-sc-icon-box.type2.overlay-icon .icon-content h4:after { background: rgba('.$rgba.', 0.9) }';
		...
		if( !empty( $secondary_color ) ) {
			...
			$rgba = mount_resort_hex2rgb( $secondary_color );
			$rgba = implode(',', $rgba);
		...
		if( !empty( $tertiary_color ) ) {
			...
			$rgba = mount_resort_hex2rgb( $tertiary_color );
			$rgba = implode(',', $rgba);
 ...
 * --------------------------------------------------------------------------- */
function mount_resort_ssl( $echo = false ){
	$ssl = '';
	if( is_ssl() ) $ssl = 's';
	if( $echo ){
		echo ($ssl);
 ...
 * --------------------------------------------------------------------------- */
function mount_resort_body_classes( $classes ) {
	
	// layout
	$classes[] 		= 	'layout-'. get_theme_mod( 'site-layout', mount_resort_defaults('site-layout') );
	...
	$htrans 		= 	get_theme_mod( 'header-transparency', mount_resort_defaults('header-transparency') );
	if(isset($htrans)):
		$classes[]	=	get_theme_mod( 'header-transparency', mount_resort_defaults('header-transparency') );
	endif;	

	$stickyhead = (int) get_theme_mod( 'enable-sticy-nav', mount_resort_defaults('enable-sticy-nav') );
	if( !empty( $stickyhead ) ) {
		$classes[]	=	'sticky-header';
	}
	
	$standard		=	get_theme_mod( 'header-position', mount_resort_defaults('header-position') );
	...
	if( is_page() ) {
		$pageid = mount_resort_ID();	
		$page_meta = get_post_meta( $pageid, '_dt_page_settings', true );
		$page_meta = is_array( $page_meta ) ? $page_meta  : array();
		
		if( array_key_exists('show-slider', $page_meta) && $page_meta['show-slider'] ) {
			$classes[] = "page-with-slider";
		}

		if( array_key_exists('show-breadcrumb', $page_meta) && !$page_meta['show-breadcrumb'] ) {
...
add_filter( 'body_class', 'mount_resort_body_classes' );?>