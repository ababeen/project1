<?php
class Mount_Resort_Twitter extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_tweetbox', 'description'=>esc_html__('To show latest twitter tweets', 'mountresort'));
		parent::__construct(false,MOUNT_RESORT_THEME_NAME.esc_html__(' Twitter Widget','mountresort'),$widget_options);
	}

	#2.widget input form in back-end
	function form($instance) {
		$instance = wp_parse_args( (array) $instance,array( 'title' => esc_html__('Latest Tweets','mountresort'), 'count' => '3', 'username' => '',
        ...
        <p><label for="<?php echo ($this->get_field_id('title')); ?>"><?php esc_html_e('Title:','mountresort');?> 
		   <input class="widefat" id="<?php echo ($this->get_field_id('title')); ?>" name="<?php echo ($this->get_field_name('title')); ?>" 
            type="text" value="<?php echo esc_attr($title); ?>" /></label></p>

        <p><label for="<?php echo ($this->get_field_id('consumer_key')); ?>"><?php esc_html_e('Consumer Key:','mountresort');?> 
        ...
        <p><label for="<?php echo ($this->get_field_id('consumer_secret')); ?>"><?php esc_html_e('Consumer Secret:','mountresort');?> 
        ...
        <p><label for="<?php echo ($this->get_field_id('access_token')); ?>"><?php esc_html_e('Access Token:','mountresort');?> 
        ...
        <p><label for="<?php echo ($this->get_field_id('access_token_secret')); ?>"><?php esc_html_e('Access Token Secret:','mountresort');?> 
        ...
        <p><label for="<?php echo ($this->get_field_id('username')); ?>"><?php esc_html_e('Enter your twitter username:','mountresort');?>
        ...
        <p><label for="<?php echo ($this->get_field_id('count')); ?>"><?php esc_html_e('How many entries do you want to show:','mountresort');?>
        ...
        <p><input type="checkbox"  id="<?php echo ($this->get_field_id('exclude_replies'));?>" name="<?php echo ($this->get_field_name('exclude_replies'));?>" 
			<?php checked($exclude_replies); ?> /> <label for="<?php echo ($this->get_field_id('exclude_replies')); ?>"><?php esc_html_e('Exclude @replies','mountresort');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo ($this->get_field_id('time'));?>" name="<?php echo ($this->get_field_name('time'));?>" 
			<?php checked($time); ?> /> <label for="<?php echo ($this->get_field_id('time')); ?>"><?php esc_html_e('Show time of tweet','mountresort');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo ($this->get_field_id('display_avatar'));?>" name="<?php echo ($this->get_field_name('display_avatar'));?>" 
				<?php checked($display_avatar); ?> /> <label for="<?php echo ($this->get_field_id('display_avatar')); ?>"><?php esc_html_e('Show user avatar','mountresort');?></label></p>		
<?php
	}
	...
	function widget($args, $instance) {
		...
		echo ($before_widget);
			...
			if($username && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) { 
					...
					require_once(MOUNT_RESORT_CORE_PLUGIN."/apis/twitteroauth/twitteroauth.php");
				...
				if($twitter && is_array($twitter)) {
					...
					echo '<li>'.esc_html__('No public Tweets found','mountresort').'</li>';
				}
				echo "</ul>";
			}
		echo ($after_widget);
