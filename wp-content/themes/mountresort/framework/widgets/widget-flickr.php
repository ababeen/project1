<?php 
class Mount_Resort_Flickr extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_flickr', 'description'=>esc_html__('A widget that show last flickr photo streams', 'mountresort'));
		parent::__construct(false,MOUNT_RESORT_THEME_NAME.esc_html__(' Flickr Widget','mountresort'),$widget_options);
	}
	...
	function form($instance) {
        ...
        <p><label for="<?php echo ($this->get_field_id('flickr_id')); ?>"><?php esc_html_e('Flickr ID:','mountresort');?>
        ...
        <p><label for="<?php echo ($this->get_field_id('count')); ?>"><?php esc_html_e('How many entries do you want to show:','mountresort');?>
         ...
         <p><label for="<?php echo ($this->get_field_id('show'));?>"><?php esc_html_e('What pictures to display','mountresort'); ?>
         	<select class="widefat" id="<?php echo ($this->get_field_id('show')); ?>" name="<?php echo ($this->get_field_name('show')); ?>">
            <?php  $a = array("latest"=>esc_html__("Latest",'mountresort'),"random"=>esc_html__("Random",'mountresort'));
         ...
         <p><label for="<?php echo ($this->get_field_id('size'));?>"><?php esc_html_e('What pictures to display','mountresort'); ?>
         	<select class="widefat" id="<?php echo ($this->get_field_id('size')); ?>" name="<?php echo ($this->get_field_name('size')); ?>">
            <?php  $a = array("s"=>esc_html__("square",'mountresort'),"t"=>esc_html__("thumbnail",'mountresort'), "m" => esc_html__("medium",'mountresort'));
			foreach($a as $key => $value ):
