<?php
class Mount_Resort_Recent_Posts extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array('classname'=>'widget_recent_posts', 'description'=>esc_html__('To list out posts', 'mountresort'));
		parent::__construct(false,MOUNT_RESORT_THEME_NAME.esc_html__(' Posts','mountresort'),$widget_options);
	}
	...
	function form($instance) {
	    ...
	    <p><label for="<?php echo ($this->get_field_id('_post_categories')); ?>">
			<?php esc_html_e('Choose the categories you want to display (multiple selection possible)','mountresort');?></label>
        ...
        <p><label for="<?php echo ($this->get_field_id('_excerpt')); ?>"><?php esc_html_e('Display title only or title &amp; excerpt','mountresort');?></label>
        ...
        <p><input type="checkbox"  id="<?php echo ($this->get_field_id('_enabled_image'));?>" name="<?php echo ($this->get_field_name('_enabled_image'));?>"
	         <?php checked($_enabled_image); ?> /> <?php esc_html_e("Show Image",'mountresort');?></p>  

	    <p><label for="<?php echo ($this->get_field_id('_post_count')); ?>"><?php esc_html_e('No.of posts to show:','mountresort');?></label>
	...
	function widget($args, $instance) {
		...
		echo "<div class='recent-posts-widget'><ul>";
			 ...
			 while($the_query->have_posts()):
				...
				echo "<li>";
					if(1 == $_enabled_image):
						...
						$image = ( $image != false)? $image[0] : MOUNT_RESORT_THEME_URI . '/images/dummy-images/post-thumb.jpg';
					...
					if($show_title):
						...
						echo "<div class='entry-title'><h4><a href='".get_permalink()."'>{$title}</a></h4></div>";
						echo "<div class='entry-content'>".mount_resort_excerpt(10)."</div>";
					endif;
			 ...
			 endwhile;
			 	...
			 	echo "<li><h4>".esc_html__('No Posts found','mountresort')."</h4></li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";
		echo ($after_widget);
